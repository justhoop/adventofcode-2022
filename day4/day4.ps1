...
function PartOne {
	...
	foreach ($pair in $pairs) {
		$first = $pair.Split(",")[0]
		$second = $pair.split(",")[-1]
		[int]$firstStart = $first.split("-")[0]
		[int]$firstEnd = $first.split("-")[-1]
		[int]$secondStart = $second.split("-")[0]
		[int]$secondEnd = $second.split("-")[-1]
		if (($firstStart -le $secondStart) -and ($firstEnd -ge $secondEnd)) {
			$count++
		}
		elseif (($secondStart -le $firstStart) -and ($secondEnd -ge $firstEnd)) {
			$count++
		}
	}
	return $count
...
function PartTwo {
	...
	foreach ($pair in $pairs) {
		...
		$first = @()
		$second = @()
		for ($i = $firstStart; $i -le $firstEnd; $i++) {
			$first += $i
		}
		for ($i = $secondStart; $i -le $secondEnd; $i++) {
			$second += $i
		}
		if ((compare-object $first $second -IncludeEqual -ExcludeDifferent).inputobject) {
			$count++
		}
	}
	return $count
...
$pairs = Get-Content .\day4.txt
$PartOne = PartOne -pairs $pairs
Write-Host "part one"$PartOne
$PartTwo = PartTwo -pairs $pairs
