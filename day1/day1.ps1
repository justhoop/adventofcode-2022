...
function PartOne {
    ...
    for ($i = 0; $i -le $calories.Count; $i++) {
        if ($calories[$i]) {
            $elfFood += [int]$calories[$i]
        }
        elseif((-not ($calories[$i])) -or ($i = $calories.Count)) {
            $elves += @{$id = $elfFood }
            $elfFood = 0
            $id++
        }
    }
    return ($elves.GetEnumerator() | Sort-Object -Property Value)
}
...
function PartTwo {
    ...
    $totalCalories = (($elves | Select-Object -Last 3).Value | Measure-Object -sum).Sum
