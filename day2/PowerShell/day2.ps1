...
function PartOne {
	...
	foreach ($round in $rounds) {	
		$elfPlays = (Get-Variable -Name $round.Split(" ")[0]).Value
		$iPlay = (Get-Variable -Name $round.Split(" ")[-1]).Value
	
		$score += $iPlay
	
		if (($elfPlays -eq 3) -and ($iPlay -eq 1)) {
			$score += 6
		}
		elseif (($elfPlays -eq 1) -and ($iPlay -eq 3)) {
			$score += 0
		}
		elseif ($iPlay -gt $elfPlays) {
			$score += 6
		}
		elseif ($iPlay -eq $elfPlays) {
			$score += 3
		}	
	}
	return $score	
...
function PartTwo {
	...
	foreach($round in $rounds){
		...
		$newRounds += $elfPlays + " " + $iPlay.$elfPlays
	}
	$score = PartOne -rounds $newRounds
	return $score
}
